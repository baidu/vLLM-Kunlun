...
# Optional Kunlun custom op imports
#
# Optimization: Fused MoE gate kernel (torch.ops._C.minimax_m3_moe_gate).
# Replaces `self.gate(hidden_states)` with a single fused kernel that computes
# bf16 input x f32 weight -> f32 output, avoiding intermediate tensor allocations
# and Python dispatch overhead.
# Performance: ~0.5-2% throughput gain on MiniMax M2.7 (256 experts top-8, TP=8, P800 XPU).
# Accuracy verified: GPQA Diamond 89.9%, AIME26 93.33% (E13 config).
try:
    import kunlun_ops  # noqa: F401
    torch._dynamo.allow_in_graph(kunlun_ops.qkrmsnorm)
except ImportError:
    kunlun_ops = None
try:
    from vllm_kunlun.platforms import envs as kunlun_envs
except ImportError:
    kunlun_envs = None


def _kunlun_moe_gate_enabled(hidden_states, weight):
    """Check if fused MoE gate kernel can be used."""
    if kunlun_envs is None:
        return False
    return (
...
def _kunlun_moe_gate(hidden_states, weight):
    """Fused MoE gate: bf16 input x f32 weight -> f32 output."""
...
class MiniMaxM2MoE(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Use fused MoE gate kernel on Kunlun XPU when available (bf16 input + f32 weight).
        # Falls back to stock PyTorch path on other platforms.
