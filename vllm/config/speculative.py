    ...
    def _verify_args(self) -> Self:
        ...
        if self.target_model_config:
            for cfg in (
                ...
                if model_type:
                    target_model_ids.append(str(model_type).lower())
                target_model_ids.extend(
                    str(architecture).lower()
                    for architecture in (getattr(cfg, "architectures", []) or [])
                )
        if (
            self.method in ("eagle3", "extract_hidden_states", "dflash")
            and self.target_model_config
            and not any(
                supported_model in model_id
            ...
            raise ValueError(
                ...
                f" models. Got target model identifiers {target_model_ids!r}"
            )
