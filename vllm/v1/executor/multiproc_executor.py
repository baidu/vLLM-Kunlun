...
class MultiprocExecutor(Executor):
    ...
    def _init_executor(self) -> None:
        ...
        # [p800-profiler-init-executor]
        # EngineCore 是 spawn 子进程，不继承 APIServer 的 env vars，
        # 必须在此重设 profiling env，后续 spawn worker 才能继承。
        # 仅在 profiling 启动（XPU_ENABLE_PROFILER_TRACING=1）时生效，
        # 普通生产启动完全不受影响。
        import os as _profiler_os
        if _profiler_os.environ.get("XPU_ENABLE_PROFILER_TRACING") == "1":
            _profiler_os.environ.setdefault("XPU_ENABLE_PROFILER_TRACING", "1")
            _profiler_os.environ.setdefault(
                "VLLM_TORCH_PROFILER_DIR", "/home/minimax_m3/Torch_profiler"
            )
            _profiler_os.environ.setdefault("XMLIR_FORCE_USE_XPU_GRAPH", "0")
...
class WorkerProc:
    ...
    def make_worker_process(
        ...
        with numa_utils.configure_subprocess(
            ...
            # [p800-profiler-patched]
            # spawn 前设置，子进程 import torch_xmlir 时读到正确单设备 id。
            # 仅在 profiling 启动（XPU_ENABLE_PROFILER_TRACING=1）时生效，
            # 普通生产启动不会给 worker 设置 XPU_CUPTI_ENABLE_DEVICE。
            import os as _spawn_os
            if _spawn_os.environ.get("XPU_ENABLE_PROFILER_TRACING") == "1":
                _spawn_os.environ["XPU_CUPTI_ENABLE_DEVICE"] = str(local_rank)
            proc.start()
