...
try:
    from vllm.model_executor.layers.shared_fused_moe import SharedFusedMoE  # noqa: F401
...
class Glm4MoE(nn.Module):
    ...
    def __init__(
        ...
        # Always use FusedMoE for routed experts (NOT SharedFusedMoE).
        # SharedFusedMoE internally calls torch.ops.vllm.moe_forward_shared
        # which is a CUDA-only op unsupported on P800 Kunlun.
        # Shared experts are computed separately in forward().
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Routed experts via FusedMoE (P800 kunlun kernel)
        final_hidden_states = self.experts(
            hidden_states=hidden_states, router_logits=router_logits
        )
        final_hidden_states = final_hidden_states * self.routed_scaling_factor

        # Shared experts computed separately (standard MLP path)
