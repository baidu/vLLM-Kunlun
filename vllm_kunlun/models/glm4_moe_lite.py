...
try:
    from vllm._aiter_ops import rocm_aiter_ops  # type: ignore
except Exception:  # pragma: no cover - optional dependency
    class _DummyAiterOps:
        def is_fusion_moe_shared_experts_enabled(self) -> bool:
            return False

    rocm_aiter_ops = _DummyAiterOps()
...
except ModuleNotFoundError:  # pragma: no cover - older vllm layout
...
import os
if os.environ.get("DUMP_ROPE_CACHE", "0") == "1":
    import sys
    sys.path.insert(0, os.environ.get("DUMP_ROPE_CACHE_DIR", "/workspace/data/tmp_test"))
    import dump_rope_cache
    dump_rope_cache.install()
